...
revenues_dt <-
  revenues_dt[, .("revenues_mean" = weighted.mean(x = revenues, w = rep)), by = .(phi, kappa)]
...
#creation of data.frame to be used to plot
revenues_df <-
...
setwd(here("fig"))
tools::texi2pdf("Figure_2.tex", clean = T)
setwd(here())

#opens the pdf plot
system(paste(
  getOption("pdfviewer"),
  here("fig/Figure_2.pdf")
))